...
export function VocabularyModule({ level, onAddPoints, onComplete, onBack, onCelebration }: VocabularyModuleProps) {
  ...
  const canAdvanceCurrentWord = normalizedTyped === normalizedTarget
  ...
  const getWordImage = (word: string, category: string) => {
    ...
    const emojiMap: Record<string, string> = {
      ...
      water: '💧', food: '🍎', family: '👨‍👩‍👧', happy: '😊', sad: '😢',
      run: '🏃', walk: '🚶', eat: '🍽️', drink: '🥛', mother: '👩', father: '👨',
      brother: '👦', sister: '👧', red: '🔴', blue: '🔵', green: '🟢',
      one: '1️⃣', two: '2️⃣', three: '3️⃣', four: '4️⃣', five: '5️⃣',
      six: '6️⃣', seven: '7️⃣', eight: '8️⃣', nine: '9️⃣', ten: '🔟',
    }
    const emoji = emojiMap[token] || (category === 'numbers' ? '🔢' : category === 'animals' ? '🐾' : '📚')
    const hue = category === 'numbers' ? 210 : category === 'animals' ? 140 : 35
    const safeLabel = word.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
    const svg = `
      <svg xmlns="http://www.w3.org/2000/svg" width="160" height="120" viewBox="0 0 160 120">
        ...
        <text x="80" y="98" text-anchor="middle" font-family="Arial, sans-serif" font-size="14" fill="#1f2937">${safeLabel.slice(0, 14)}</text>
      </svg>`
    ...
    <div className="vocabulary-module">
      ...
      {showHelpGuide && (
        <div className="vocab-help-card">
          <p><strong>Cómo usar:</strong> selecciona un nivel, escucha la palabra, escribe la palabra exacta y luego presiona “Siguiente”.</p>
          <p>Las palabras de números aparecen en una sección separada cuando el nivel incluye números.</p>
        </div>
      )}
      ...
      <div className={`word-card ${justLearned ? 'just-learned' : ''}`}>
        ...
        <div className="word-content">
          <div className="word-visual-frame">
            <img src={getWordImage(currentWord.english, currentWord.category)} alt={`Imagen para ${currentWord.english}`} className="word-visual-image" />
          </div>
